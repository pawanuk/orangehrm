...
interface FieldData {
  ...
  selector: string;
  value?: string;
  fieldType?: 'text' | 'textarea' | 'dropdown' | 'checkbox' | 'radio' | 'date' | 'file';
}
...
const getSelector = (field: FieldData): string => {
  const baseSelector = `//label[text()='${field.label}']/following-sibling::`;
...
const fillForm = async (page: Page, fields: FieldData[]) => {
  for (const field of fields) {
    const value = generateRandomData(field);
    ...
    if (element) {
      ...
      switch (field.fieldType) {
        ...
        case 'textarea':
          ...
          break;
        case 'dropdown':
          await element.selectOption({ value: field.value || value }); // Use provided value or generated value
          break;
        case 'checkbox':
        case 'radio':
          await element.check(field.value || true); // Check the box (optional value for pre-selection)
          break;
        case 'date':
    ...
    if (element) {
      ...
      switch (field.fieldType) {
        ...
        case 'number':
          ...
          break;
        case 'dropdown':
          fieldValue = await element.evaluate(el => el.selectedOptions[0].textContent);
          break;
        case 'checkbox':
        case 'radio':
...
const verifyFormData = (formData: Record<string, string>, expectedData: Record<string, string>) => {
  for (const [label, expectedValue] of Object.entries(expectedData)) {
    ...
    expect(actualValue).toEqual(expectedValue, `Field "${label}" mismatch. Expected: "${expectedValue}", Found: "${actualValue}"`);
  }
};
...
test('Fill and verify form using random data', async ({ page }) => {
  ...
  for (const field of fields) {
    expectedFormData[field.label] = generateRandomData(field);
  }
