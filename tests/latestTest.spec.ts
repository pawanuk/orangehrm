...
const extractFormFields = async (page: Page): Promise<FieldData[]> => {
    return await page.evaluate(() => {
        ...
        labels.forEach((label) => {
            ...
            if (labelText) {
                const commonDiv = label.closest('div'); // Adjust selectors for different form layouts

                if (commonDiv) {
                    const input = commonDiv.querySelector('input, textarea, select');
                    if (input) {
                        const fieldType = input.tagName.toLowerCase() === 'select' ? 'dropdown' : input.getAttribute("type");
                        if (fieldType) {
                            fields.push({
                                label: labelText,
